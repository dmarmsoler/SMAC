// docker run --rm -v C:\Users\aa1558\contracts:/contracts solc-verify:latest '/contracts/assignd2n5.sol'
 /**Test configurations: 
 *  #Dimensions (D) : D = 2
 *   Size       (n) : n = 5
 *  Value-to-memory : Yes 
 *   Single Aliasing: No 
 *   Double Aliasing: No
 *    Initialization: No
 */
 /* Test result:
 assignd2n5::assignd2n5f: OK
assignd2n5::[implicit_constructor]: OK
No errors found.
 */

pragma solidity ^0.5.0;

contract assignd2n5 {
  
/// @notice precondition i < 5
/// @notice precondition j < 5
/// @notice postcondition x[i][j] == value 
  function assignd2n5f(uint8[5][5] memory x, uint8 i, uint8 j, uint8 value) 
    pure
    public 
    returns (uint8[5][5] memory)  
    {
        x[i][j] = value;
      return x;
    }

}